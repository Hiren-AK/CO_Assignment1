...
for i in range(len(inp)):
  ...
  #sub_____________________________
    ...
    #ld_____________________________
      ...
      if not isBinary(inp[i][3]) and inp[i][3] not in variables.keys():
        if inp[i][3] in labels.keys():
          errors.append(("Misuse of labels as variables or vice-versa", i+1))
          break

        elif inp[i][3] not in variables.keys():
    ...
    #jmp_____________________________(CHECK)
      ...
      if not isBinary(inp[i][3]) and inp[i][3] not in labels.keys():
  ...
  #ld_____________________________
    ...
    if not isBinary(inp[i][2]) and inp[i][2] not in variables.keys():
        if inp[i][2] in labels.keys():
          errors.append(("Misuse of labels as variables or vice-versa", i+1))
          break

        elif inp[i][2] not in variables.keys():
            errors.append(("Use of undefined variables", i+1))
            break
  ...
  #jmp_____________________________(CHECK)
    ...
    if not isBinary(inp[i][1]) and inp[i][1] not in labels.keys():
